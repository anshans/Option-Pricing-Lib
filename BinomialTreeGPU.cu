...
void BinomialTree::createPriceTree(){
    ...
    cudaMalloc(&layersOfNodes, steps*sizeof(PriceNode**));
    ...
    fillPriceTree<<<1,8>>>(layersOfNodes, steps, u, d);
};
__global__ void fillPriceTree(PriceNode** layersOfNodes, int steps, float u, float d){
    for(int parentLayer = 0; parentLayer < steps - 1; parentLayer++){
        ...
        layersOfNodes[parentLayer+1] = new PriceNode[BinomialTree::numInLayer(parentLayer+1)];

        for(int nodeInParentLayer = 0; nodeInParentLayer < BinomialTree::numInLayer(parentLayer); nodeInParentLayer++){
...
static int BinomialTree::numInLayer(int layerNum){
